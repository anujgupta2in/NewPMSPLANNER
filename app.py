...
def display_machinery_breakdown(df):
    ...
    machinery_details = pending_df.groupby('Machinery Location').agg({
        ...
        'Calculated Due Date': lambda x: pd.to_datetime(x, errors='coerce').min().date() if pd.to_datetime(x, errors='coerce').notna().any() else None
    })
...
def display_running_hours(df, data_processors=None):
    ...
    # Keep only rows that have a real running hours value (not null and not zero)
    general_df = general_df[general_df['Machinery Running Hours'].notna() & (general_df['Machinery Running Hours'] > 0)]
...
DATE RANGE:
Earliest Due Date: {pd.to_datetime(df['Calculated Due Date'], errors='coerce').min().date() if pd.to_datetime(df['Calculated Due Date'], errors='coerce').notna().any() else 'N/A'}
Latest Due Date: {pd.to_datetime(df['Calculated Due Date'], errors='coerce').max().date() if pd.to_datetime(df['Calculated Due Date'], errors='coerce').notna().any() else 'N/A'}
